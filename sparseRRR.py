...
"""
"""
# Functions from: https://github.com/berenslab/patch-seq-rrr
#%%
import numpy as np
import time
import warnings
...
def bibiplot(X, Y, w, v, 
    ...
    if XdimsToShow.size > 0:
        ...
        for i in XdimsToShow:
            plt.plot([0, scaleFactor*L[i,0]], [0, scaleFactor*L[i,1]], linewidth=1.2, color='black')
            plt.grid(False)
            plt.text(scaleFactor*L[i,0]*1.2, scaleFactor*L[i,1]*1.2, XdimsNames[i], 
                     ha='center', va='center', color='black')
        circ = plt.Circle((0,0), radius=scaleFactor, color='black', fill=False, linewidth=1)
...
#            Ytest  = (Ytest-Y_mean) / Y_std
            ...
            for i,a in enumerate(lambdas):    
                for j,b in enumerate(alphas):
                    ...
                    #r2[cvfold, rep, i, j] = np.sqrt(np.sum((Ytest - Xtest @ vx @ vy.T)**2)/Ytest.shape[0])
                    for r in range(rank):
                    ...
                    #r2_relaxed[cvfold, rep, i, j] = np.sqrt(np.sum((Ytest - Xtest @ vx @ vy.T)**2)/Ytest.shape[0])
                    for r in range(rank):
